...
cackey_chrome_returnType cackey_chrome_decryptMessage(struct cackey_certificate *certificate, void *data, unsigned long dataLength, void *destination, unsigned long *destinationLength, char **pinPrompt, const char *pin) {
	...
	CK_MECHANISM decryptMechanism = {CKM_RSA_PKCS_OAEP, NULL, 0};
	CK_OBJECT_CLASS objectClassPrivateKey = CKO_PRIVATE_KEY;
	CK_OBJECT_CLASS objectClassCertificate = CKO_CERTIFICATE;
	CK_TOKEN_INFO tokenInfo;
	CK_ULONG tmpDestinationLength;
	...
	*pinPrompt = NULL;
	...
	for (currSlot = 0; currSlot < numSlots; currSlot++) {
		...
		if (foundPrivateKeyObject) {
			chk_rv = moduleFunctionList->C_DecryptInit(hSession, &decryptMechanism, hKey);
			if (chk_rv != CKR_OK) {
				break;
			}

			tmpDestinationLength = *destinationLength;
			chk_rv = moduleFunctionList->C_Decrypt(hSession, data, dataLength, destination, &tmpDestinationLength);
			switch (chk_rv) {
				case CKR_OK:
					*destinationLength = tmpDestinationLength;
					retval = CACKEY_CHROME_OK;
